...
class Line {
    ...
    allLinkPointsXY: { x: number; y: number; item: InstanceNodePoint }[];
    ...
    calcLinkPoint = (x: number, y: number, adsorb = [20, 20]) => {
        const newXY = this.allLinkPointsXY.find((item) => {
            if (
                ...
                this.hoverLinkPoint = item.item;
                item.item.dom?.classList.add("hover");
                return item;
            }
        });
        if (!newXY) {
    ...
    makeAdsorbPoints = () => {
        ...
        for (let key in nodes) {
            ...
            linkPoints.forEach((item) => {
                const x = parseInt(item.dom.getAttribute("cx"));
                const y = parseInt(item.dom.getAttribute("cy"));
